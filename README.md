...
Version 1.4.0 adds device-specific button glyphs and reactive Xbox, Steam Deck and ROG Ally diagrams to the modern TriPad interface. Controller Setup shows live presses, trigger travel and stick movement alongside the selected action's binding. Radial, keyboard, refit and status prompts use the same device appearance and effective mappings. Starsector's Insignia fonts, native icons and gameplay behavior stay intact. See [the design and offscreen previews](docs/UI_REFINEMENT.md).

The original [native HUD frame skin](docs/HUD_SKIN.md) uses mod textures and the session's player UI palette, without editing installed game assets. Use one native skin at a time: AashPad and similar skins can conflict with these texture paths and colors. Controller discovery, mouse-compatible touch and optional Steam Input gyro introduced in 1.2.0 remain available; see [handheld setup](docs/HANDHELDS_AND_DIAGNOSTICS.md#steamos-with-lutriswine-or-proton).

**Validation status, 8 September 2026:** the mod builds and passes its automated checks. The 1.4.0 UI has offscreen OpenGL renders with the actual runtime drawing and installed Insignia fonts at 720p, 800p and 1080p, with input geometry and GL state-restoration checks. Earlier isolated Windows game tests cover text entry, cargo pickup/cancellation, profiles, wheels, tactical orders, autopilot, Console and failure cleanup. The user confirmed physical X pause/unpause and campaign fleet movement. The refined theme has not had a new in-game or physical handheld acceptance pass; controller combat, physical touch/gyro and SteamOS operation remain unverified. See [compatibility and evidence](docs/COMPATIBILITY.md).
...
In **LunaLib > SectorPad > General > Device appearance**, Automatic uses a recognizable controller name. Steam Input/Wine can hide the physical model behind XInput or Steam Virtual Gamepad; select **Steam Deck**, **ROG Ally** or **Xbox** manually in that case. This saved appearance changes only the diagram and button artwork. It does not select a device or change bindings.

Touchscreen taps activate SectorPad controls on release, and vertical drags scroll its setup, refit and keyboard lists without also selecting a row. The radial menu follows a drag and commits the highlighted wedge on release. Steam Input can expose a handheld gyro as mouse motion; enable **Gyro mouse mode** in LunaLib to use those deltas for combat aim. Xbox controllers have no gyro sensor, so gyro settings apply to handheld motion or another sensor exposed by the platform.
