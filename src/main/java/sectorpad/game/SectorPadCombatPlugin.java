...
public final class SectorPadCombatPlugin extends BaseEveryFrameCombatPlugin {
    @Override public void processInputPreCoreControls(float amount, List<InputEventAPI> events) {
        ...
        if (RuntimeHooks.isEnabled() && Global.getCurrentState() == GameState.COMBAT) OverlayHost.get().renderCombatAboveHud(Global.getCombatEngine());
        } catch (RuntimeException | LinkageError failure) { RuntimeHooks.fail("runtime.combat_input", failure); }
    }
    @Override public void renderInUICoords(ViewportAPI viewport) {
        ...
        if (!RuntimeHooks.isEnabled()) return;
        try { if (Global.getCurrentState() == GameState.COMBAT) OverlayHost.get().beginCombatFrame(Global.getCombatEngine()); }
        catch (RuntimeException | LinkageError failure) { RuntimeHooks.fail("runtime.combat_overlay", failure); }
    }
}
