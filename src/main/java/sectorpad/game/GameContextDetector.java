    ...
    public GameContext detect() {
        ...
        if (game == GameState.CAMPAIGN) {
            ...
            String identity = stateIdentity + ":" + StateAccess.identity(sector) + ":" + tab + ":"
                    ...
                    + ":" + ui.isShowingDialog() + ":" + ui.isShowingMenu() + ":" + codex;
            boolean free = !ui.isShowingDialog() && !ui.isShowingMenu() && tab == null && !modal && !codex && nativeModal == null;
            if (free) return new GameContext("CAMPAIGN", identity, true, sector.isPaused(), "Campaign");
            if (tab == CoreUITabId.MAP && (dialog == null || core != null) && !ui.isShowingMenu() && !codex
